...
def verify_url(url: str, timeout: int = 5) -> dict:
    ...
    Returns:
        Dict with 'valid' (bool), 'status_code' (int or None), 'error' (str or None)
    """
    try:
        parsed = urlparse(url)
        ...
        return {
            ...
            "error": None if is_valid else f"Status code: {response.status_code}"
        }
...
def verify_urls_batch(urls: list[str]) -> dict:
    """
    Verify multiple URLs and return validation results.
    
    Args:
        urls: List of URLs to verify
    
    Returns:
        Dict mapping URL to verification result
    """
    results = {}
    for url in urls:
        if url and isinstance(url, str) and url.strip():
            results[url] = verify_url(url.strip())
        else:
            results[url] = {
                "valid": False,
                "status_code": None,
...
def legal_search(
    ...
    """Search for legal information across various sources with URL verification."""
    ...
    if "results" in search_results and isinstance(search_results["results"], list):
        urls_to_verify = [result.get("url") for result in search_results["results"] if result.get("url")]
        verification_results = verify_urls_batch(urls_to_verify)
        
        valid_results = []
        for result in search_results["results"]:
            url = result.get("url")
            if url and verification_results.get(url, {}).get("valid", False):
                valid_results.append(result)
        
        search_results["results"] = valid_results
        search_results["url_verification"] = verification_results
...
def case_law_search(
    ...
    """Search specifically for case law and judicial precedents with URL verification."""
    court_keywords = {
...
def statutory_search(
    ...
    """Search for statutes, acts, and legislative provisions with URL verification."""
    act_keywords = {
...
NOTE: All search tools now automatically verify URLs before returning results. Only valid, accessible URLs will be included in search results."""
...
CRITICAL: All URLs returned by search tools have been automatically verified for accessibility. Only valid, working URLs are included in results. You must still capture URLs from search results for reference purposes.
...
IMPORTANT: All search tools now automatically verify URLs before returning results. URLs that are inaccessible or return errors are automatically filtered out. You will only receive valid, working URLs in search results. You must still include these verified URLs in your final response.
...
- Include URLs whenever available from search results (all URLs are pre-verified for accessibility)
...
def research_legal_query_streaming(
    query: str, 
    files: Optional[dict] = None,
    mode: Literal["normal", "detailed"] = "normal"
) -> Iterator[dict]:
    """
    Stream legal research progress and final response as it generates.
    ...
    Yields:
        ...
        - {'type': 'status', 'data': {'node': str, 'message': str}}
        - {'type': 'progress', 'data': {'node': str, 'details': dict}}
        - {'type': 'content', 'data': str} (streaming JSON response)
        - {'type': 'complete', 'data': str} (final JSON)
        - {'type': 'error', 'data': str}
    """
    ...
    yield {
        ...
        'data': {
            'node': 'initialization',
            'message': f'Legal Research Agent - {mode.upper()} MODE with URL verification'
        }
    }
    ...
    try:
                ...
                for node_name, node_data in chunk.items():
                    yield {
                        ...
                        'data': {
                            'node': node_name,
                            'details': {
                                'files_updated': list(node_data.get('files', {}).keys()) if isinstance(node_data, dict) and 'files' in node_data else [],
                                'message_count': len(node_data.get('messages', [])) if isinstance(node_data, dict) and 'messages' in node_data else 0
                            }
                        }
                    }
        ...
        if final_result:
            ...
            if response_content:
                ...
                error_response = json.dumps({"error": "No response generated"})
                yield {
                    'type': 'error',
                    'data': error_response
                }
...
if __name__ == "__main__":
    ...
    print(f"\nTesting STREAMING mode with URL verification")
    print(f"Query type: complex")
    print(f"Mode: {mode_to_test}")
    print(f"Query: {query_to_test}\n")
    
    print("\n" + "=" * 80)
    print("STREAMING OUTPUT".center(80))
    print("=" * 80 + "\n")
    
    for event in research_legal_query_streaming(query_to_test, mode=mode_to_test):
        if event['type'] == 'status':
            print(f"[STATUS] {event['data']['node']}: {event['data']['message']}")
        elif event['type'] == 'progress':
            print(f"[PROGRESS] {event['data']['node']}")
            if event['data']['details']['files_updated']:
                print(f"  Files: {', '.join(event['data']['details']['files_updated'])}")
            if event['data']['details']['message_count']:
                print(f"  Messages: {event['data']['details']['message_count']}")
